...
# K331 is a exceptional case

def get331():
    # k331 input
    k331 = np.array([[0, 0, 0, 0]])
    # print(k331)
    # Read in and grasp useful info
    with open('k331.csv', 'r', newline='') as f:
        ...
        for row in reader:
            ...
            if nprow[2] == ' Note_on_c':
                ...
                # print(nprow)
                useful_row = np.array(
                    [[round((int(nprow[1]) - 1152) / 96), int(nprow[3]), int(nprow[4]) + 3, int(nprow[5])]])
                    # from A to C
                # print(useful_row)
                k331 = np.concatenate((k331, useful_row), axis=0)

    k331 = k331[1:]
    # print(k545)

    # Sort by time
    k331 = k331[k331[:, 0].argsort()]
    # print(k331)
    ...
    for j in range(3304):
        ...
        while i < len(k331) and k331[i][0] == j:
            if not k331[i][1]:  # right hand
                if r_i < 5:
                    new_r_roll[0][r_i] = k331[i][2]
                r_i += 1
            else:  # left hand
                # print(i, l_i)
                new_l_roll[0][l_i] = k331[i][2]
                l_i += 1
            i += 1
        new_r_roll.sort()
        new_l_roll.sort()
