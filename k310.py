...
def get310():
    # k310 input
    k310 = np.array([[0, 0, 0, 0]])
    # print(k310)
    # Read in and grasp useful info
    with open('k310.csv', 'r', newline='') as f:
        ...
        for row in reader:
            ...
            if nprow[2] == ' Note_on_c':
                ...
                # print(useful_row)
                k310 = np.concatenate((k310, useful_row), axis=0)
    ...
    for j in range(2905):
        ...
        while i < 4602 and k310[i][0] == j:
            if not k310[i][1]:  # right hand
                new_r_roll[0][r_i] = k310[i][2]
                r_i += 1
            else:  # left hand
                # print(i, l_i)
                new_l_roll[0][l_i] = k310[i][2]
                l_i += 1
            i += 1
        new_r_roll.sort()
        new_l_roll.sort()
